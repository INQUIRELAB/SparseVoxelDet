#!/usr/bin/env python3
"""
SparseVoxelDet V82 — Fully Sparse Event-Based Object Detector.

V82 changes vs original:
  1. Default in_channels=6 (6-channel temporal surface features)
  2. Default input_size=(720, 1280) (native sensor resolution)
  3. Default time_bins=16 (power-of-2)
...
Design decisions (critically evaluated):
  1. Why NOT SparseInverseConv3d for upsampling?
     - Requires indice_key from backbone's forward strided conv, which doesn't set it.
     - SparseConvTranspose3d is used instead -- creates its own upsampling pattern.
  2. Why fuse to stride-4 instead of stride-8?
     - At 640x640, stride-8 gives 80x80. A 10px drone = 1.25 cells. Too small.
     - At stride-4: 160x160. A 10px drone = 2.5 cells. Viable.
     - At 1280x720 native: stride-4 gives 320x180. Even better.
  3. Why squeeze temporal dimension before head?
     - Backbone processes 3D (T, H, W) but detection is 2D (H, W).
     - After backbone, temporal dim is redundant (features already encode temporal info).
     - Squeeze via max-pool over T, then use 2D sparse operations for the head.
     - Actually: spconv uses 3D throughout; the head must also use 3D sparse ops.
     - Temporal dim collapses through strided convolutions; at stride-4 with T=16 bins
       and stem_stride=(1,2,2), T stays at 16. We squeeze T at the FPN output.
"""
import math
