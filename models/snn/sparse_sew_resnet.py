...
TRUE sparse convolutions - only non-zero voxels are processed.
...
class SparseBasicBlock(nn.Module):
    ...
    def __init__(
        ...
        if stride > 1:
            ...
            # CRITICAL: Use (1, stride, stride) to only downsample spatial dimensions
            # Scalar stride would be (stride, stride, stride), collapsing temporal!
    ...
    def forward(self, x: spconv.SparseConvTensor) -> spconv.SparseConvTensor:
        ...
        # CRITICAL FIX: Sort indices before strided conv
        # spconv strided conv produces wrong results when indices have
        # specific ordering from SubMConv operations. Sorting indices fixes this.
        if self.stride > 1:
...
class SparseDownsample(nn.Module):
    ...
    def forward(self, x: spconv.SparseConvTensor) -> spconv.SparseConvTensor:
        # CRITICAL FIX: Sort indices before strided conv
        # spconv produces wrong results with certain index orderings
...
class SparseSEWResNet(nn.Module):
    ...
    Processes sparse event voxels using TRUE sparse convolutions.
