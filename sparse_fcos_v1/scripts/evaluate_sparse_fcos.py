...
# Lazy imports — these are only needed when running full evaluation,
# not when importing individual functions like temporal_rerank_top1
def _lazy_imports():
    from sparse_fcos_v1.scripts.sparse_event_dataset import (
        SparseEventDataset, make_collate_fn, create_sparse_tensor
    )
    from sparse_fcos_v1.scripts.metrics import MAPCalculator, compute_map_at_iou_range
    return SparseEventDataset, make_collate_fn, create_sparse_tensor, MAPCalculator, compute_map_at_iou_range
