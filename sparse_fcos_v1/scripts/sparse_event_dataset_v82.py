...
from sparse_fcos_v1.scripts.event_mosaic import sparse_mosaic
...
class SparseEventDataset(Dataset):
    ...
    def __init__(
        ...
        # Find all samples
        self.causal_shift = int(os.environ.get('CAUSAL_SHIFT', 0))
    ...
    def _find_samples(self) -> List[Tuple[Path, Path]]:
        ...
        for seq_dir in sorted(self.sparse_dir.iterdir()):
            ...
            for frame_file in sorted(seq_dir.glob('frame_*.npz')):
                ...
                label_idx = frame_idx + getattr(self, 'causal_shift', 0)

                # Find corresponding label
                label_file = self.label_dir / f'{seq_id}_frame_{label_idx:06d}.txt'
                if not label_file.exists():
                    # Try without zero padding
                    label_file = self.label_dir / f'{seq_id}_frame_{label_idx}.txt'

                if label_file.exists():
                    samples.append((frame_file, label_file))
                elif getattr(self, 'causal_shift', 0) == 0:
                    missing_labels.append(str(frame_file))
                # causal_shift>0: missing (k+shift) label at seq end / annotation gap
                # is an expected, non-fatal drop (not an integrity violation)
