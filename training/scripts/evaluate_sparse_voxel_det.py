#!/usr/bin/env python3
"""
Evaluation script for SparseVoxelDet.
...
METRICS_ENGINE_ID = "sparse_voxel_det.mapcalc"
...
def _assert_parity_eval_contract(
    ...
    expected_data = (project_root / "data/datasets/fred_paper_parity_v82/sparse").resolve()
...
def run_sparse_tensor_contract_preflight(
    ...
    expected_feat_channels: Optional[int] = None,
    ...
    if expected_feat_channels is not None:
...
def select_evenly_spaced_samples(samples: List[Tuple[Path, Path]], n_keep: int) -> List[Tuple[Path, Path]]:
    """Deterministically subsample across the full split instead of taking the first N."""
    if n_keep <= 0 or n_keep >= len(samples):
        return list(samples)
    indices = np.linspace(0, len(samples) - 1, num=n_keep, dtype=np.int64)
    return [samples[int(i)] for i in indices.tolist()]
...
def evaluate(
    ...
    """
    Evaluate SparseVoxelDet detector on FRED dataset.
    ...
    # Handle different checkpoint formats — prefer EMA weights (shadow copy).
    # EMA weights are what training uses for validation and yield higher mAP.
    if 'ema_state_dict' in checkpoint:
        ema_data = checkpoint['ema_state_dict']
        state_dict = ema_data['shadow']
        print(f"  Using EMA shadow weights (decay={ema_data.get('decay', '?')})")
    elif 'model_state_dict' in checkpoint:
        state_dict = checkpoint['model_state_dict']
        print(f"  WARNING: No EMA weights in checkpoint — using raw model weights")
    ...
    eval_max_detections = int(max_detections) if max_detections is not None else int(eval_cfg.get('max_detections', 100))
    ...
    sparse_contract_cfg = data_cfg.get('sparse_contract', {}) or {}
    ...
    if sparse_contract_enabled:
        contract_report_json = (Path(outdir) / "preflight_sparse_tensor_contract_eval.json") if outdir else None
        run_sparse_tensor_contract_preflight(
            sparse_root=data_dir,
            splits=[split],
            ...
            expected_feat_channels=model_config.get('in_channels', None),
    ...
    if max_samples:
        ...
        dataset.samples = select_evenly_spaced_samples(dataset.samples, int(max_samples))
        if len(dataset.samples) < orig_n:
            sample_selection_mode = "evenly_spaced_subset"
            print(
                f"Subsampled evaluation set deterministically across full split: "
                f"{len(dataset.samples)}/{orig_n} samples"
            )

    collate_fn = make_collate_fn(time_bins=time_bins, base_size=(int(input_size[0]), int(input_size[1])))
    ...
    print(f"  SparseVoxelDet:        {results['mAP@50'] * 100:.2f}%")
    print(f"  Gap:                   {gap:.2f}%")
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate SparseVoxelDet')
    ...
    data_dir = args.data_dir or 'data/datasets/fred_paper_parity_v82/sparse'
    label_dir = args.label_dir or 'data/datasets/fred_paper_parity/labels'
    split = args.split or 'canonical_test'
