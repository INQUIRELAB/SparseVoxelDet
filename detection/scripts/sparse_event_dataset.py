#!/usr/bin/env python3
"""
Sparse Event Dataset for FCOS Training.

Loads pre-processed sparse event tensors and labels for training
the SparseFCOSDetector. Handles:

1. Loading sparse coordinates and features from .npz files
2. Loading YOLO-format labels
3. Data augmentation (horizontal flip, random crop, event dropout)
4. Collation into batched sparse tensors

The dataset expects data prepared by prepare_sparse_tensors.py.
"""
import torch
import numpy as np
...
class SparseEventDataset(Dataset):
    ...
    def _load_sparse(self, path: Path) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if 'coords' in data and 'feats' in data:
            ...
            feats = data['feats'].astype(np.float32)   # [N, 2] = (on, off)
        elif 'spikes' in data:
    ...
    def _augment_polarity_flip(
        ...
        """Swap ON/OFF channels (columns 0 and 1)."""
        feats = feats.copy()
        feats = feats[:, ::-1].copy()
        return feats
...
def _collate_batch(
    ...
    Args:
        ...
        target_size: Spatial size for this batch (for multi-scale training).
        base_size: Original spatial size the data was generated at.

    Returns:
        Dictionary with coords, feats, spatial_shape, gt_boxes, gt_labels, batch_size
    """
    batch_size = len(batch)
    # Handle tuple target/base sizes (non-square native resolution like 720x1280)
    if isinstance(target_size, (tuple, list)) and isinstance(base_size, (tuple, list)):
        ts = tuple(int(x) for x in target_size)
        bs = tuple(int(x) for x in base_size)
        if ts == bs:
            scale = 1.0
        else:
            scale = (ts[0] / bs[0], ts[1] / bs[1])  # (scale_h, scale_w)
    else:
    ...
    for batch_idx, sample in enumerate(batch):
        ...
        feats = sample['feats']            # [N_i, 2]
        boxes = sample['boxes'].clone()    # [M, 4]

        # Scale spatial coordinates if multi-scale
        if scale != 1.0:
            # Scale y and x
            coords[:, 1] = (coords[:, 1].float() * scale).int()
            coords[:, 2] = (coords[:, 2].float() * scale).int()
            # Clip to target_size
            coords[:, 1] = coords[:, 1].clamp(0, target_size - 1)
            coords[:, 2] = coords[:, 2].clamp(0, target_size - 1)
            # Scale boxes
            if len(boxes) > 0:
                boxes = boxes * scale
    ...
    feats = torch.cat(all_feats, dim=0)    # [N_total, 2]

    # Determine time_bins
    if time_bins is None:
        # Infer from data (V1 behavior - can fail if batch has no events in last bins)
        if coords.shape[0] > 0:
    ...
    if isinstance(target_size, (tuple, list)):
        _h, _w = int(target_size[0]), int(target_size[1])
    else:
        _h = _w = int(target_size)
    spatial_shape = [int(time_bins), _h, _w]
...
def make_collate_fn(
    ...
    Args:
        ...
        multi_scale_sizes: List of spatial sizes to randomly pick from per batch.
            E.g., [512, 576, 640, 704, 768]. None = fixed 640.
        base_size: Original spatial size the data was generated at.
    ...
    def collate_fn(batch: List[Dict]) -> Dict:
        if multi_scale_sizes is not None:
            target_size = random.choice(multi_scale_sizes)
        else:
